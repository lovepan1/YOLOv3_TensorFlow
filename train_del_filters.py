...
# Model definition
##################
yolo_model = sparse_yolov3(args.class_num, args.anchors, args.use_label_smooth, args.use_focal_loss, args.batch_norm_decay, args.weight_decay)
with tf.variable_scope('yolov3'):
    pred_feature_maps = yolo_model.forward_include_res(image, is_training=is_training)
...
saver_to_restore = tf.train.Saver(var_list=tf.contrib.framework.get_variables_to_restore(include=['yolov3/yolov3_head']))
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        # print("loss_total.last_avg is ", loss_total.last_avg )
        ...
        #     # preci_myself = float(right_obj / float(obj_sum))
        #     # print("pre_total is :{:.4f}".format(prec_total.sum))
        #     # print("prec_avg is :{:.4f}".format(prec_total.avg))
        #     # print("self cals pre is:{:.4f}".format(preci_myself))
        #     pre = prec_total.avg
        #     mAP = ap_total.avg
        ...
        #     logging.info(info)
        #
        #     if mAP > best_mAP or pre > best_pre:
        #         best_mAP = mAP
        #         best_pre = pre
        ...
        saver_best.save(sess, 'prun_checkpoint/prun_include_res_model')
