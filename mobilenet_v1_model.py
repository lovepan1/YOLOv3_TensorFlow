...
from utils.layer_mobilenet_utils import darknet53_mobilenet_body, conv_block_mobilet, yolo_block_mobilenet
import args
...
class mobilenet_yolov3(object):
    ...
    def predict(self, feature_maps):
        ...
        center_x, center_y, width, height = tf.split(boxes, [1, 1, 1, 1], axis=-1, name='predict')
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('yolov3_head'):
                    ...
                    '''
                    boxes, scores, labels = self.post_process((feature_map_1, feature_map_2, feature_map_3))
    ...
    def post_process(self, featuremaps):
        # feature_map_1, feature_map_2, feature_map_3 = featuremaps
        pred_boxes, pred_confs, pred_probs = self.predict(featuremaps)
        pred_scores = pred_confs * pred_probs
        boxes, scores, labels = gpu_nms(pred_boxes, pred_scores, self.class_num, max_boxes=30,
                                        score_thresh=0.4, nms_thresh=0.5)
        return boxes, scores, labels
