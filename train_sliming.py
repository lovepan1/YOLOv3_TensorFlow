...
update_vars = tf.contrib.framework.get_variables_to_restore(include=['yolov3/darknet53_body','yolov3/yolov3_head'])
        ...
        if args.sliming:
            for i, (g, v) in enumerate(grads_varlist):
                if g is not None:
                    if 'gamma' in str(v):
                        print(v)
                        grads_varlist[i] += args.s * np.linalg.norm(sess.run(v))
                        print("gamma loss is ", args.s * np.linalg.norm(sess.run(v)) )
